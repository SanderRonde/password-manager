...
function rewriteEsModuleImports(file: string|Buffer): string {
	return file.toString()	
		...
		.replace(/import (.*) from ['"]wclib['"]/g, 'import $1 from \'/modules/wclib/wclib.js\'')
...
export function initDevelopmentMiddleware(webserver: Webserver) {
	...
	webserver.app.all([
		'/modules/u2f-api',
		'/modules/u2f-api.js',
		'/modules/u2f-api/u2f-api.js'
	], async (_req, res) => {
		const result = await getWebpackPacked('u2fApi',
			path.join(PROJECT_ROOT, 'node_modules/u2f-api/dist/lib/u2f-api.js'));
		res.contentType('.js');
		res.write(`${result};
	...
	webserver.app.use(serve(path.join(PROJECT_ROOT, 'node_modules/wclib/build/es/'), {
		exclude: ['/serviceworker.js'],
		extensions: ['js'],
		rewrite: rewriteEsModuleImports,
		prefix: '/modules/wclib/build/es/'
	}));
	webserver.app.use(serve(path.join(PROJECT_ROOT, 'node_modules/wclib/build/es/'), {
		exclude: ['/serviceworker.js'],
		extensions: ['js'],
		rewrite: rewriteEsModuleImports,
		prefix: '/modules/wclib/'
	}));
